...
path <- "../Coral_fastq/Pl/Primer_Adaptor_Removed" # CHANGE to the directory containing your demultiplexed fastq files
...
filterEmpty <- out[,2] != 0
saveRDS(filterEmpty,"./Output/Pl/filterEmpty.RDS")
out <- out[filterEmpty,] # Filtering out empty samples
saveRDS(out,"./Output/Pl/out.RDS")
#out <- readRDS("./Output/Pl/out.RDS")
...
filts <- filts[filterEmpty]
errF <- learnErrors(filts, multithread=TRUE, MAX_CONSIST = 20)
saveRDS(errF,"./Output/Pl/errF.RDS")
#errF <- readRDS("./Output/Pl/errF.RDS")
...
saveRDS(dadaFs,"./Output/Pl/dadaFs.RDS")
dadaFs <- readRDS("./Output/Pl/dadaFs.RDS")
...
head(track)
write.csv(track, file = "./Output/Pl/read_counts_at_each_step.csv", row.names = TRUE)


# Save intermediate seqtable object
saveRDS(seqtab.nochim, "./Output/Pl/seqtab.nochim.RDS")
# Resuming Point ##############################
#seqtab.nochim <- readRDS("./Output/Pl/seqtab.nochim.RDS")
#filterEmpty <- readRDS("./Output/Pl/filterEmpty.RDS")
...
saveRDS(seqtab.nochim, file = "./Output/Pl/dada2_seqtable.RDS")
saveRDS(taxa, file = "./Output/Pl/RDP_Taxonomy_from_dada2.RDS")
...
# The following data will need to be edited to shift the read counts away from the blanks as it assumes that the blank samples are the first rows of the previous data frame.
nBlanks <- 6
